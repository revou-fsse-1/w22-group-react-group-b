...
export default function CategoryRecipe() {
	...
	return (
		<div className="flex flex-row flex-wrap gap-5 justify-evenly">
			<div>
				<div className="mt-24 flex flex-row items-center  text-lg">
					<ul className="w-fit">
					...
					{western.map((data) => {
						...
						return (
							<Link
								...
								className="rounded-xl text-black flex font-bold text-2xl text-center italic"
								key={data.id}
								style={{
									background: imgURL,
			...
			<div>
				<div className="mt-24 flex flex-row items-center  text-lg">
					...
					<span className="text-black ml-2">Sweets</span>
				</div>
				<div className="flex flex-col gap-4">
					{sweet.map((data) => {
						const imgURL = `linear-gradient(rgba(236, 236, 236, 0.6), rgba(236, 236, 236, 0.6)), url('${data.imageURL}')`;
